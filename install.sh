...
if [ ${GR_VER} = "3.10" ]; then
    ...
    sudo apt-get install gnuradio gnuradio-dev gr-osmosdr librtlsdr-dev libuhd-dev libhackrf-dev liborc-dev cmake git build-essential pkg-config doxygen clang-format python3-pybind11 python3-numpy python3-requests libsndfile1-dev libspdlog-dev libportaudio2 python3-sounddevice $([ "$FORCE" = true ] && echo "-y")
