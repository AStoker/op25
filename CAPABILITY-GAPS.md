...
- [ ] **[P1] `adj_tune` — fine tuning.** Advertised: "ability to adjust fine tuning in real time (,./<> keys)".
...
- [ ] **[P1] `set_debug` — "Dynamically controllable log level".** Handler at
...
- [ ] **[P1] `capture` — "Dynamic demodulator symbol capture and replay (commanded through terminal)".**
...
- [ ] **[P1] `reload` — "TGID Blacklist, Whitelist with **dynamic reloading**".**
...
- [ ] **[P2] `dump_tgids` / `dump_buffer`.** Handlers at
...
- [ ] **[P2] `lockout` / `whitelist` are wired in the service but no component calls them.**
...
- [ ] **[P2] No way to enter an arbitrary TGID.** Curses has `H` (hold/goto tgid), `W` (whitelist tgid),
...
- [ ] **[P3] `get_ws_instances`** ([multi_rx.py:912](op25/gr-op25_repeater/apps/multi_rx.py#L912)) is
      never requested. Related to the per-channel audio gap in §5.

## 2. Decoder → browser messages that arrive and are dropped

- [ ] **[P1] `rx_update` is ignored.** Emitted at
...
- [ ] **[P2] `terminal_config` is requested and then discarded.** Requested at
...
- [ ] **[P2] `meta_update` is ignored.** Emitted by
      ...
      state in the new UI.
- [ ] **[P2] `SDR_STATUS` is a declared channel that is never used.** `MSG_SDR_STATUS`,
...
- [ ] **[P2] Initial `SYSTEM_STATE` snapshot is permanently `status: "stopped", uptime: 0`.**
...
- [ ] **[P1] `SYSTEM_CONTROL` mute/unmute are sent and silently discarded.** `PlayerCard` sends
...
- [ ] **[P2] `start` / `stop` / `restart` / `volume` are declared in the protocol type and unimplemented**
...
- [ ] **[P2] `set_full_config` is a stub.** [multi_rx.py:909-911](op25/gr-op25_repeater/apps/multi_rx.py#L909-L911)
...
- [ ] **[P2] `dump_tracking` has no handler at all.** Sent by both legacy UIs
...
- [ ] **[P3] `set_freq` and `add_default_config` are curses-only.** Bound at
...
- [ ] **[P1] DMR/Connect+ trunking ignores every UI command.**
...
- [ ] **[P1] Per-channel audio selection is missing.** `_discover_audio_ports()`
...
- [ ] **[P1] `ws://` destinations are ignored by the new stack.** A user who follows
...
- [ ] **[P3] `/api/stream` `rate`/`format` query params are not exposed in the UI**
...
- [ ] **[P1] SmartNet/SmartZone.** `tk_smartnet`'s `to_json` emits only
...
- [ ] **[P1] Connect+ / DMR.** [tk_trbo.py:85-95](op25/gr-op25_repeater/apps/tk_trbo.py#L85-L95) returns
...
- [ ] **[P1] DMR slot is never displayed.** `call_log` carries `slot`
      ...
      is ambiguous.
- [ ] **[P2] TGID priority is not surfaced.** "TGID Priority with mid-call preemption" is advertised and
      `call_log.prio` is on the wire; nothing renders it, and there is no UI to change it.
- [ ] **[P2] NBFM analog is a single chip.** `ChannelsCard` shows only `analog`/`digital`
...
- [ ] **[P2] Encryption state is under-reported.** Decoder-side support exists for `crypt_keys` /
...
- [ ] **[P1] NBFM noise & voice squelch (PA3FWM / DB1NV).** New `apps/squelch_core.py`,
...
- [ ] **[P1] Gate LDU2 audio on newly received AlgId when skipping encrypted calls** —
      `lib/p25p1_fdma.cc` (+10). Fixes a burst of encrypted audio leaking through `crypt_behavior=2`.
- [ ] **[P2] "Tolerate empty module names in audio/metadata/terminal config sections"** (`multi_rx.py`).
      Cheap robustness fix for configs with `"module": ""`.
- [ ] **[P3] `field-test/` kit** (11 files: NBFM squelch field-test harness, config renderer, analyzers).
      Decide whether to carry it — it is upstream's calibration tooling for the squelch work above.
- [ ] **[P3] Upstream README updates** — capability list now says "…HTTP based terminal … with websocket
      audio" plus a W1JPI squelch section. Our README has diverged (see §8).

## 8. Documentation that no longer matches the code

- [ ] **[P1] `README.md` advertises the wrong protocol doc for this branch.** Our README's
...
- [ ] **[P1] `README.md` does not advertise this fork's actual capabilities.** Nothing about the
      ...
      still upstream's.
- [ ] **[P2] `README-browser-audio.md` describes a mechanism the new UI does not implement** (ws://
      destinations, per-channel headphone toggle, "Mute Browser Audio at Startup"). Needs a "new stack"
      section or an explicit note.
- [ ] **[P2] `www/app/AGENTS.md` is thin; the new protocol is documented only in `CLAUDE.md`.**
...
- [ ] **[P3] `apps/README.md` "HTTP Console" section covers only `rx.py -l http:…`.** No mention of
...
- [ ] **[P2] `/api/config` is never fetched.** Endpoint at
...
- [ ] **[P2] `/api/ha/status` has no UI surface.** It is documented as the place to start HA
...
- [ ] **[P3] Empty leftover component directories:** `www/app/src/components/FrequenciesCard/` and
...
- [ ] **[P3] No tuning/quality feedback in the UI.** `ChannelStatus` carries `ppm`, `error`
...
- [ ] **[P3] Carry-overs already known in `CLAUDE.md`** — restated here so they live in one checklist:
